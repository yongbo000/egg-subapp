...
module.exports = app => {
  for (const subApp of app.subApps.values()) {
    ...
    // 获取所有js文件
    let routerFilePaths = fs.existsSync(subAppRouterDir) ? globby.sync([ '**/*.js' ], { cwd: subAppRouterDir }).map(filepath => {
  ...
  methods.forEach(method => {
    routerApp[method] = routerApp.router[method] = function subAppRouter(...args) {
      ...
      if (is.string(lastOne)) {
        ...
        const viewRoot = app.config.subApp.rootDir;
        const viewPath = path.join(viewRoot, subAppName, 'view', lastOne);
